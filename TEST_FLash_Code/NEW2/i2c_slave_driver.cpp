...
void I2cSlaveDriver::init(uint32_t base)
{
    ...
    // NOTE: Target(スレーブ)専用でも、内部ロジック用のプリスケーラ(I2CPSC)
    //       設定のために I2C_initController() を呼ぶのがTI公式サンプルの
    //       慣習(i2c_ex4/6/ex5 いずれも同様)。bitRate/dutyCycle引数は
    //       このデバイスが自らクロックを出すことがない(Targetのみ)ため
    //       実質未使用だが、プリスケーラ計算のために必要な値として渡す。
    I2C_initController(base_, DEVICE_SYSCLK_FREQ, 400000, I2C_DUTYCYCLE_50);

    // Target(スレーブ)受信モード / 7bitアドレッシング / 自局アドレス設定
    I2C_setConfig(base_, I2C_TARGET_RECEIVE_MODE);
    I2C_setOwnAddress(base_, I2C_SLAVE_ADDRESS);
    I2C_setAddressMode(base_, I2C_ADDR_MODE_7BITS);
    I2C_setBitCount(base_, I2C_BITCOUNT_8);
    I2C_disableLoopback(base_);
    I2C_setEmulationMode(base_, I2C_EMULATION_FREE_RUN);

    // FIFOは使わず1バイトずつ RX_DATA_RDY 割込みで処理する(基本割込みのみ使用)
    I2C_disableFIFO(base_);

    // 割込み要因:
    //   ADDR_TARGET      : 自アドレスに一致(START/RESTART)
...
//-----------------------------------------------------------------------------
// ADDR_TARGET: アドレス一致(新規START、またはRESTART後の再アドレッシング)
//-----------------------------------------------------------------------------
void I2cSlaveDriver::handleAddrTarget()
{
    // マスタからの書き込み(Target受信)以外は本ドライバの対象外
    if (I2C_getStatus(base_) & I2C_STS_TARGET_DIR)
    {
        // マスタが読み出し要求(Target送信)を出してきた場合はここでは扱わない。
        // 必要であれば I2C_setConfig(base_, I2C_TARGET_SEND_MODE) 等で
        // 別途応答処理を実装すること。
        return;
    }
