...
void main(void)
{
    ...
    if(isFwUpdateMode())
    {
        ...
        if(eraseResult != FLASH_CTRL_OK)
        {
            ...
            I2C_SetAppState(I2C_APP_STATE_ERROR);
            while(1){}
        }
        ...
        if(i2cResult != I2C_SLAVE_OK)
        {
            ...
            // I2C_FwUpdate内部（Flash書き込み失敗）でも
            // I2C_APP_STATE_ERRORはセット済みだが、
            // 将来他の失敗要因が増えた場合の保険として明示的に設定
            I2C_SetAppState(I2C_APP_STATE_ERROR);
            while(1){}
        }
        //フラグクリア
    ...
    else
    {
        ...
        // 【注意】jumpToApplication()はDINT+IER/IFRクリアを行い
        // アプリ側コードへジャンプするため、ここでI2C_APP_STATE_APP_RUNNING
        // をセットしても、ブートローダのI2C_RxISR/I2C_EventISRは
        // ジャンプ後は動作しなくなる（割り込みが無効化されるため）。
        // ジャンプ後もSTATUS_REQ(0x20)に応答したい場合は、
        // アプリ側で改めてI2Cスレーブ初期化＋状態管理を行う必要がある。
        jumpToApplication();
    }
