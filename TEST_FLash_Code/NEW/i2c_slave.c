...
void I2C_SlaveInit(void)
{
    ...
    // 【修正】基本割り込みではなく、FIFO割り込みフラグ(RXFF)をクリア
    I2C_clearInterruptStatus(I2CA_BASE, I2C_INT_RXFF);
    
#if IRC_CODE
    //    RX FIFO割り込みレベルを RX1（1バイト）に設定する。
    //    1バイト受信ごとに割り込みが発生し、ISRで1バイトずつ
    //    バッファ(s_rxBuf)に格納する。
    ...
    // 【修正】有効化する割り込みソースを I2C_INT_RXFF に変更
 ...
 *
 * 呼び出しタイミング:
 *   RX FIFOに1バイト受信されたとき（I2C_FIFO_RX1設定時）
 *
 * 処理:
 *   1バイト取得し、s_rxBuf に格納する。
...
__interrupt void I2C_RxISR(void)
{
    ...
    //
    // RX FIFO割り込みレベルを RX1 に設定しているため、
    // 割り込み1回につき1バイトのみ取得し、バッファに格納する。
    //
