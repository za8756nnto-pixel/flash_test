...
//
// TI C2000 (F280015x) I2Cスレーブ受信ドライバ
//
// 対応フレーム:
//   Format A: [SlaveAddr(0x50)+W][CMD][LEN][DATA x LEN][CRC8][STOP]
//   Format B: [SlaveAddr(0x50)+W][CMD][RESTART][SlaveAddr(0x50)+W][DATA...][CRC8][STOP]
//
// 受信はI2C割込み(AAS/RRDY/SCD/NACK)で行い、ISR内ではリングバッファへの
// push()のみを行う(重い処理はしない)。
...
extern "C" __interrupt void i2cSlaveBasicISR(void);   // I2Cの基本割込み(AAS/RRDY/SCD/NACK等)
